...
class general_functions:
    ...
    def R(z, zr):
        if z.any() == 0:
            ...
            R = np.multiply(z, 1 + np.square(np.divide(zr, z)))
        return R
    ...
    #Define function that defines the waves amplitude (depends only on spatial location)
    def amp(E_0, w_0, w, r):
        return E_0 * np.divide(w_0, w) * np.exp(-1*np.square(np.divide(r, w)))

    def amp2E(amp, phase_space, omega, t):
        return np.multiply(amp, np.cos(omega*t - phase_space))

    def amp2I(E_0, w_0, w, r):
        return np.multiply(np.divide(0.5*((E_0*w_0)**2)/(2*377), np.square(w)), np.exp(-2*np.square(np.divide(r, w))))
    ...
    def convert_coord(x, y_min, y_max, origin, alpha, delta_y):
        delta_x = x - origin[0]
        delta_y = np.arange(y_min, y_max, delta_y) - origin[1]

        z = delta_x*np.cos(alpha) - delta_y*np.sin(alpha)
        r = delta_x*np.sin(alpha) + delta_y*np.cos(alpha)

        return np.vstack((z, r))