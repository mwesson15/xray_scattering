...
if __name__=="__main__":
    ...
    # sim.run(mp.at_every(1 , mp.output_png(mp.Ez, "-Zc dkbluered")), until_after_sources=10)
    # sim.output_farfields(nearfield, "spectra-{}-{}-{}".format(d1, d2, h),
    #                  mp.Volume(mp.Vector3(0,  d2 + (0.5 * h)), size=mp.Vector3(100, h)),
    #                  resolution)

    sim.run(until_after_sources=mp.stop_when_fields_decayed(50, mp.Ez, mp.Vector3(40, 0), 1e-4))

    sim.output_farfields(nearfield, "spectra-{}-{}-{}".format(d1, d2, h), resolution,
                     where=mp.Volume(mp.Vector3(0, d2 + (0.5 * h)),
                                     size=mp.Vector3(1, 1e5)))
