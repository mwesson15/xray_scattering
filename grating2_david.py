...
d = Distance2Detector
r = d*np.tan(alpha)
thet = np.linspace(-np.pi/2,np.pi/2,101)
alph = np.concatenate((np.arange(0,np.pi/2,np.pi/100),np.linspace(np.pi/2,0,51)))
print(alph)
...
farE = np.zeros((thet.shape[0],3), dtype=np.complex128)
...
for i in tqdm(range(alph.shape[0])):
    ff = sim.get_farfield(nearfield, mp.Vector3(r*np.sin(thet[i]),-r*np.sin(alph[i]),d))
    farE[i,:] = ff[0:3]
...
farI = farE[:,0] + farE[:,1] + farE[:,2]
farI = farI.astype(np.single)
_q = (2*np.pi/wavelength)*np.sin(thet)*np.cos(alph)/10

q = (2*np.pi/wavelength)*(XValues/Radius)*np.cos(np.arcsin(YValues/Radius))/10

print('q diff', np.unique(q-_q))
print('farI diff', np.unique(farI-FarI))
